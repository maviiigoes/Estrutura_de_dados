	...
	public:
		...
		void addOrd(int m, char n[23]){
			...
			if(inicio == NULL){
				...
				No* atual = inicio;
				while(atual->matricula < m && atual->prox != NULL) {
					atual = atual->prox;
				}
				novoNo->prox = atual;
		...
		void mostraIniFim(){
			if (inicio == NULL) cout<<"\nLista vazia!!!";
				...
				No* elemento = inicio;
				while (elemento != NULL) {
					cout << elemento->matricula << ": " << elemento->nome << endl;
		...
		int remover(int m){
			if(m == inicio->matricula) {
				removerInicio();
			} else if(m == fim->matricula) {
				removerFinal();
			} else {
				No* atual = inicio->prox;

				while(atual->matricula != m) {
					if(atual->matricula > m || atual == NULL) return 0;
