...
Poli termo(float coeficiente, int expoente, Poli prox) {
...
void destruirPoli(Poli *polinomio) {
    if( (*polinomio) == NULL) return;
    destruirPoli(&(*polinomio)->prox);
    free(*polinomio);
    *polinomio = NULL;
}
